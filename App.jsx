...
// ============================================================
const C = {
  bg: "#070e1a", card: "#0c1826", card2: "#101e2e",
  sky: "#6cabdd", gold: "#f59e0b", purple: "#7c3aed",
  red: "#ef4444", muted: "#64748b", border: "#1a2d42",
  text: "#f0f8ff", sub: "#93c5fd", accent: "#38bdf8", emerald: "#6cabdd",
};
const S = {
  app:   { background: C.bg, minHeight: "100vh", fontFamily: "'Inter',system-ui,sans-serif", color: C.text },
  hdr:   { background: "linear-gradient(160deg,#040a12 0%,#0b1929 50%,#040a12 100%)", borderBottom: `1px solid ${C.border}`, padding: "18px 16px 14px", textAlign: "center" },
  nav:   { display: "flex", gap: 5, padding: "9px 10px", borderBottom: `1px solid ${C.border}`, background: C.card, overflowX: "auto" },
  nb:    (a) => ({ padding: "7px 13px", borderRadius: 7, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 11, background: a ? C.sky : "#0f1e30", color: a ? "#070e1a" : C.muted, whiteSpace: "nowrap", transition: "all 0.15s" }),
  card:  { background: C.card, borderRadius: 12, border: `1px solid ${C.border}`, padding: 14, marginBottom: 10 },
  inp:   { background: "#070e1a", border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, padding: "9px 12px", fontSize: 13, width: "100%", outline: "none", boxSizing: "border-box", appearance: "none" },
    ...
    <div style={{ display: "flex", gap: 3 }}>
      {form.map((r, i) => (
        <div key={i} style={{
          ...
          background: r === "W" ? C.sky : r === "D" ? C.gold : C.red,
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 8, fontWeight: 800, color: "#070e1a",
        }}>{r}</div>
      ))}
    </div>
  );
...
function EVCard({ title, data }) {
  if (!data) return null;
  const col = data.verdict === "VALUE" ? C.sky : data.verdict === "SMALL VALUE" ? C.gold : data.verdict === "FAIR" ? C.accent : C.red;
  return (
    <div style={{ background: "#070e1a", border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, marginTop: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3px 0", fontSize: 11 }}>
        {"modelProbability" in data && <>
          ...
          <span style={{ textAlign: "right", fontWeight: 800, color: data.edgePct >= 0 ? C.sky : C.red }}>
            {data.edgePct > 0 ? "+" : ""}{data.edgePct}%
          </span>
        </>}
        {"evPct" in data && <>
          <span style={{ color: C.muted }}>Full Win</span><span style={{ textAlign: "right", color: C.sky }}>{data.fullWin}%</span>
          ...
          <span style={{ textAlign: "right", fontWeight: 800, color: data.evPct >= 0 ? C.sky : C.red }}>
            ...
            <div key={team} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {isNew && <span style={{ fontSize: 9, fontWeight: 800, background: "1a2d42", color: C.sky, padding: "1px 6px", borderRadius: 99 }}>↑ NEW</span>}
              ...
              <div style={{ height: 5, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${pct}%`, background: i === 0 ? C.gold : i < 5 ? C.sky : i < 10 ? C.accent : C.muted, borderRadius: 3, transition: "width 0.5s" }} />
              </div>
            </div>
          );
        })}
        <div style={{ marginTop: 12, padding: "8px 10px", background: "#070e1a", borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 5, display: "flex", gap: 6, alignItems: "center" }}>
              ...
              {NEW_TEAMS.has(home) && <span style={{ fontSize: 9, color: C.sky, fontWeight: 700 }}>↑ PROMOTED</span>}
            </div>
          </div>
        ...
        <div style={S.card}>
          <div style={{ textAlign: "center", marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 6, color: C.muted, fontSize: 11 }}>
              ...
              {" · "}Confidence: <strong style={{ color: result.confidence === "High" ? C.sky : result.confidence === "Medium" ? C.gold : C.muted }}>{result.confidence}</strong>
              {result.blended && <span style={{ marginLeft: 6, color: C.sky, fontSize: 10 }}>🔀 Odds blended</span>}
            </div>
          </div>
    ...
    <div style={{ padding: 12 }}>
      <div style={S.card}>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[true, false].map(h => (
              <button key={String(h)} onClick={() => setIsHome(h)} style={{ flex: 1, padding: "8px 0", borderRadius: 7, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 11, background: isHome === h ? C.sky : "#0f1e30", color: isHome === h ? "#070e1a" : C.muted }}>
      ...
      {prediction && home !== away && (
        <>
          <div style={{ background: "linear-gradient(135deg,#071426,#0a1e36)", border: `2px solid ${C.sky}`, borderRadius: 14, padding: 18, textAlign: "center", marginBottom: 10 }}>
            <div style={{ color: C.sky, fontSize: 10, fontWeight: 800, letterSpacing: 2, marginBottom: 8 }}>PREDICTED SCORELINE</div>
            ...
            <div style={{ color: C.muted, fontSize: 11 }}>
              ...
              {" · "}Confidence: <strong style={{ color: prediction.confidence === "High" ? C.sky : prediction.confidence === "Medium" ? C.gold : C.muted }}>{prediction.confidence}</strong>
            </div>
          </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ display: "flex", gap: 3, height: 6, borderRadius: 3, overflow: "hidden", marginBottom: 6 }}>
              <div style={{ flex: prediction.probabilities.homeWin, background: C.sky }} />
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 12 }}>
              <span style={{ color: C.sky }}>{prediction.probabilities.homeWin}% Home</span>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              <StatBox label={`${home.split(" ")[0]} xG`} value={prediction.xG[home]} color={C.sky} />
  ...
  return (
    <div style={{ padding: 12 }}>
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ ...S.lbl, marginBottom: 0 }}>
            ...
            {liveCount > 0 && <span style={{ marginLeft: 8, background: C.sky, color: "#070e1a", borderRadius: 99, padding: "1px 7px", fontSize: 10 }}>{liveCount} LIVE</span>}
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => setAutoRefresh(a => !a)} style={{ ...S.btn, marginTop: 0, flex: 1, background: autoRefresh ? "#0a1e36" : "#0f1e30", color: autoRefresh ? C.sky : C.muted }}>
      ...
      {matches.map(m => {
        ...
        return (
          <div key={m.id} style={{ ...S.card, borderColor: isLive ? C.sky : C.border, background: isLive ? "#061a0e" : C.card, position: "relative", overflow: "hidden" }}>
            {isLive && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg,${C.sky},#93c5fd,${C.sky})` }} />}
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: C.muted, marginBottom: 8 }}>
              ...
              <span style={{ color: isLive ? C.sky : isDone ? C.muted : C.gold, fontWeight: isLive ? 800 : 400 }}>
                {isLive && <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: C.sky, marginRight: 4 }} />}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ minWidth: 64, textAlign: "center" }}>
                {hasScore ? <span style={{ fontSize: 20, fontWeight: 900, color: isLive ? C.sky : C.accent }}>{m.homeScore} – {m.awayScore}</span>
...
function OddsView() {
  ...
  const ModelEdge = ({ prob, odds }) => {
    ...
    const col = edge >= 5 ? C.sky : edge >= 2 ? C.gold : edge >= -2 ? C.muted : C.red;
  ...
  return (
    <div style={{ padding: 12 }}>
      <div style={S.card}>
        ...
        {savedKey && <button onClick={() => fetchOdds(savedKey)} style={{ ...S.btn, background: "#0f1e30", color: C.sky }}>🔄 Refresh</button>}
        <div style={{ marginTop: 6, fontSize: 10, color: usingLive ? C.sky : C.gold }}>
        ...
        <div key={m.id} style={S.card}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {[{ label: m.home.split(" ")[0], odds: m.h2h.home, prob: getModelP(m.home, m.away, "home") },
                  ...
                  <div key={item.label} style={{ flex: 1, background: "#070e1a", border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 6px", textAlign: "center" }}>
          ...
          {m.totals?.slice(0, 1).map((t, i) => (
            <div key={i} style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              <div style={{ flex: 1, background: "#070e1a", border: `1px solid ${C.border}`, borderRadius: 7, padding: "7px 10px", display: "flex", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[["📊 1X2", "h2h"], ["⚽ O/U", "total"], ["🧮 AH", "ah"]].map(([label, type]) => (
              ...
              }} style={{ flex: 1, background: "#0f1e30", border: `1px solid ${C.border}`, color: C.accent, borderRadius: 7, padding: "6px 0", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                {label}
              </button>
            ))}
          </div>
            ...
            return (
              <div key={opp} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderTop: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ color: pred.probabilities.homeWin >= 50 ? C.sky : C.gold, fontSize: 10 }}>{pred.probabilities.homeWin}%</span>
                </div>
              </div>
            );
          })}
    ...
    <div style={{ padding: 12 }}>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          {[{ label: "PLAYED", value: played.length }, { label: "CORRECT", value: correct.length, color: C.sky }, { label: "ACCURACY", value: `${acc}%`, color: acc >= 55 ? C.sky : acc >= 40 ? C.gold : C.red }, { label: "SCORE HITS", value: played.filter(m => m.scoreCorrect).length, color: C.gold }].map(s => (
            <div key={s.label} style={{ flex: 1, background: "#070e1a", borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
        ...
        {played.length > 0 && (
          <div style={{ height: 4, borderRadius: 2, background: C.border, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${acc}%`, background: acc >= 55 ? C.sky : acc >= 40 ? C.gold : C.red, borderRadius: 2 }} />
          </div>
        )}
      </div>
      <div style={{ display: "flex", gap: 5, marginBottom: 12 }}>
        {[["all","All"], ["correct","✅ Correct"], ["wrong","❌ Wrong"]].map(([id, label]) => (
          <button key={id} onClick={() => setFilter(id)} style={{ flex: 1, padding: "7px 0", borderRadius: 7, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 11, background: filter === id ? C.sky : "#0f1e30", color: filter === id ? "#070e1a" : C.muted }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {filtered.map(m => (
        <div key={m.id} style={{ ...S.card, borderColor: m.outcomeCorrect ? "#065f46" : "#7f1d1d", background: m.outcomeCorrect ? "#050f07" : "#0f0505" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 6 }}>
            ...
            <span style={{ color: m.outcomeCorrect ? C.sky : C.red, fontWeight: 800 }}>
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <div style={S.card}>
        ...
        {savedKey && !loading && (
          <button onClick={() => fetchLiveIntel(savedKey)} style={{ ...S.btn, background: "#0f1e30", color: C.sky }}>
            🔄 Refresh Intel
          </button>
        )}
        <div style={{ marginTop: 8, fontSize: 10, color: usingLive ? C.sky : C.gold }}>
          {usingLive ? "✅ Live lineups & injuries from API" : "⚡ Manual mode — add any match below"}
        </div>
        {error && <div style={{ color: error.startsWith("✅") ? C.sky : C.red, fontSize: 11, marginTop: 6 }}>{error}</div>}
      </div>
          ...
          <div key={matchKey} style={S.card}>
              ...
              <div>
                ...
                <div style={{ color: C.muted, fontSize: 10, marginTop: 3 }}>
                  ...
                  {match.homeLineup?.confirmed
                    ? <span style={{ color: C.sky, marginLeft: 6 }}>✅ Lineup confirmed</span>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
              <button onClick={() => applyOverride(matchKey, match)}
                style={{ flex: 1, background: hasOverride ? "#0a1e30" : "#0f1e30", border: `1px solid ${hasOverride ? C.sky : C.border}`, color: hasOverride ? C.sky : C.accent, borderRadius: 7, padding: "7px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                {match.news?.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    ...
                    {match.news.map((n, i) => (
                      <div key={i} style={{ fontSize: 11, color: C.muted, padding: "5px 8px", background: "#070e1a", borderRadius: 6, marginBottom: 4 }}>{n}</div>
                    ))}
                  </div>
                )}
                ...
                <div style={{ marginBottom: 10 }}>
                  {predictions[matchKey] ? (
                    ...
                    <button onClick={() => getFullPrediction(matchKey, match)} disabled={predLoading[matchKey]}
                      style={{ ...S.btn, marginTop: 0, background: predLoading[matchKey] ? "#0f1e30" : "#0a2a1a", color: predLoading[matchKey] ? C.muted : C.sky }}>
                ...
                {/* AI briefing */}
                {aiSummary[matchKey] ? (
                  <div style={{ background: "#07101e", border: `1px solid #1a2a3e`, borderRadius: 10, padding: 12 }}>
                  ...
                  <button onClick={() => getAiBriefing(matchKey, match)} disabled={aiLoading[matchKey]}
                    style={{ ...S.btn, background: "#0f1e30", color: C.accent }}>
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ ...S.lbl, marginBottom: 0 }}>
            ...
            {usingLive && <span style={{ marginLeft: 8, color: C.sky, fontSize: 9, fontWeight: 700 }}>● LIVE</span>}
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => setAutoRefresh(a => !a)} style={{ ...S.btn, marginTop: 0, flex: 1, background: autoRefresh ? "#0a1e30" : "#0f1e30", color: autoRefresh ? C.sky : C.muted }}>
            {autoRefresh ? "⏱ Auto ON" : "⏱ Auto"}
          </button>
        </div>
        {autoRefresh && <div style={{ color: C.sky, fontSize: 10, marginTop: 5 }}>✅ Auto-refresh every 2 minutes</div>}
      ...
      <div style={{ ...S.card, padding: "10px 8px", overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10, minWidth: 0 }}>
          <thead>
            <tr>
            ...
            {computedStandings.map((row, i) => {
              ...
              return (
                <tr key={row.team}
                  ...
                  style={{ cursor: "pointer", background: highlight === row.team ? "#0a1e36" : "transparent", transition: "background 0.1s" }}>
                  ...
                  <td style={{ ...S.td, textAlign: "center", color: C.sky, fontWeight: row.W > 0 ? 800 : 400 }}>{row.W}</td>
                  ...
                  <td style={{ ...S.td, textAlign: "center", color: row.GD > 0 ? C.sky : row.GD < 0 ? C.red : C.muted, fontWeight: 700 }}>
      ...
      <div style={{ ...S.card, background: "#07101e", borderColor: "#1a2a3e" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 0", fontSize: 11 }}>
          ...
          <span style={{ color: C.muted }}>Promoted</span><span style={{ textAlign: "right", color: C.sky }}>Coventry · Ipswich · Hull</span>
...
// ============================================================
// TAB: TOP SCORERS — Live search via AI + ESPN fallback
// ============================================================
const FALLBACK_SCORERS = [
  ...
  { rank:9,  name:"A. Semenyo",      team:"Bournemouth",       goals:0, assists:0 },
  { rank:10, name:"B. Fernandes",    team:"Manchester United", goals:0, assists:0 },
  { rank:11, name:"F. Wirtz",        team:"Liverpool",         goals:0, assists:0 },
  { rank:12, name:"M. Kudus",        team:"West Ham United",   goals:0, assists:0 },
  { rank:13, name:"Rayan",           team:"Bournemouth",       goals:0, assists:0 },
  { rank:14, name:"E. Ferguson",     team:"Brighton",          goals:0, assists:0 },
  { rank:15, name:"W. Gnonto",       team:"Leeds United",      goals:0, assists:0 },
  { rank:16, name:"C. Wood",         team:"Nottingham Forest", goals:0, assists:0 },
  { rank:17, name:"D. Kulusevski",   team:"Tottenham Hotspur", goals:0, assists:0 },
  { rank:18, name:"M. Belloumi",     team:"Hull City",         goals:0, assists:0 },
  { rank:19, name:"R. Cherki",       team:"Manchester City",   goals:0, assists:0 },
  { rank:20, name:"E. Eze",          team:"Crystal Palace",    goals:0, assists:0 },
];
...
function TopScorersView() {
  ...
  const [loading, setLoading]     = useState(false);
  const [lastUpdate, setLastUpdate] = useState(null);
  const [view, setView]           = useState("goals"); // "goals" | "assists"
  ...
  const fetchScorers = async () => {
    ...
    try {
      const resp = await fetch(
        "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/leaders"
      );
      if (resp.ok) {
        const data = await resp.json();
        const goalsLeaders = data?.leaders?.find(l =>
          l.name?.toLowerCase().includes("goal") || l.abbreviation === "G"
        );
        const assistLeaders = data?.leaders?.find(l =>
          l.name?.toLowerCase().includes("assist") || l.abbreviation === "A"
        );
        if (goalsLeaders?.leaders?.length) {
          const parsed = goalsLeaders.leaders.map((p, i) => ({
            rank: i + 1,
            name: p.athlete?.displayName || p.athlete?.shortName || "Unknown",
            team: p.team?.displayName || p.team?.shortDisplayName || "",
            goals: p.value || 0,
            assists: 0,
          }));
          setScorers(parsed.slice(0, 20));
          if (assistLeaders?.leaders?.length) {
            const parsedA = assistLeaders.leaders.map((p, i) => ({
              rank: i + 1,
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          tools: [{ type: "web_search_20250305", name: "web_search" }],
          messages: [{ role: "user", content:
            `Search for "Premier League 2026-27 top scorers golden boot" and return the current top 20 goalscorers. Also search for "Premier League 2026-27 top assists" for the top 10 assist makers.
Return ONLY valid JSON, no markdown, no explanation:
{"scorers":[{"rank":1,"name":"Player Name","team":"Club","goals":5,"assists":2}],"assists":[{"rank":1,"name":"Player Name","team":"Club","assists":3,"goals":1}]}
If season hasn't started yet, return empty arrays.` }]
        })
      });
      const data = await resp.json();
      const text = data.content?.filter(c => c.type === "text").map(c => c.text || "").join("") || "";
      ...
      if (parsed.scorers?.length) {
        ...
        if (parsed.assists?.length) setAssists(parsed.assists);
      } else {
    ...
    <div style={{ padding: 12 }}>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {lastUpdate && <span style={{ color: C.muted, fontSize: 10 }}>{fmtTime(lastUpdate)}</span>}
            <button onClick={fetchScorers} disabled={loading}
              style={{ background: "#0f1e30", border: `1px solid ${C.border}`, color: C.sky, borderRadius: 7, padding: "5px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
              {loading ? "⏳" : "🔄"}
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[["goals","⚽ Goals"],["assists","🅰 Assists"]].map(([id, label]) => (
            <button key={id} onClick={() => setView(id)} style={{ flex: 1, padding: "8px 0", borderRadius: 7, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 12, background: view === id ? C.sky : "#0f1e30", color: view === id ? "#070e1a" : C.muted }}>
              {label}
            </button>
          ))}
        </div>
        <div style={{ color: C.muted, fontSize: 10, marginTop: 6 }}>
          ESPN + AI web search · {seasonStarted ? "Live data" : "Season starts 21 Aug 2026 — pre-season rankings shown"}
        </div>
      </div>

      {/* Season not started notice */}
      {!seasonStarted && !loading && (
        <div style={{ ...S.card, background: "#070e1a", borderColor: C.border, textAlign: "center", padding: 16 }}>
          <div style={{ fontSize: 11, color: C.gold, marginBottom: 4 }}>📅 Season starts 21 Aug 2026</div>
          <div style={{ fontSize: 11, color: C.muted }}>Showing predicted key players. Goals will update live once the season begins.</div>
        </div>
      )}

      {/* Scorers list */}
      {loading && !displayList.length && (
        <div style={{ ...S.card, textAlign: "center", color: C.muted, padding: 28 }}>⏳ Loading...</div>
      )}
      ...
      {displayList.map((s, i) => {
        ...
        return (
          <div key={`${s.name}-${i}`} style={{
            ...S.card,
            padding: "10px 12px",
            borderColor: isMCity ? C.sky : C.border,
            background: isMCity ? "#0a1826" : C.card,
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{
                ...
                background: i === 0 ? "#fbbf24" : i === 1 ? "#94a3b8" : i === 2 ? "#cd7f32" : "#0f1e30",
                color: i < 3 ? "#070e1a" : C.muted,
              }}>{s.rank}</div>

              {/* Player info */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: isMCity ? C.sky : C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {s.name}
                  {isMCity && <span style={{ marginLeft: 6, fontSize: 9, color: C.sky }}>🩵</span>}
                </div>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: i === 0 ? C.gold : C.sky, lineHeight: 1 }}>
                  {view === "assists" ? s.assists : s.goals}
                </div>
                <div style={{ fontSize: 9, color: C.muted, marginTop: 2 }}>
                  {view === "assists" ? "AST" : "GLS"}
                </div>
                {view === "goals" && s.assists > 0 && (
                  <div style={{ fontSize: 9, color: C.muted }}>🅰 {s.assists}</div>
                )}
                {view === "assists" && s.goals > 0 && (
                  <div style={{ fontSize: 9, color: C.muted }}>⚽ {s.goals}</div>
                )}
              </div>
            </div>
            ...
            {statVal > 0 && (
              <div style={{ marginTop: 8, height: 3, background: C.border, borderRadius: 2, overflow: "hidden" }}>
                <div style={{
                  ...
                  width: `${Math.min(100, (statVal / (displayList[0] ? (view === "assists" ? displayList[0].assists : displayList[0].goals) : 1)) * 100)}%`,
                  background: i === 0 ? C.gold : isMCity ? C.sky : C.accent,
                }} />
              </div>
            )}
          </div>
      ...
      {/* Golden Boot info */}
      <div style={{ ...S.card, background: "#07101e", borderColor: "#1a2d42" }}>
        <div style={S.lbl}>🏅 AWARD HISTORY</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px 0", fontSize: 11 }}>
          <span style={{ color: C.muted }}>2025/26 Golden Boot</span>
          <span style={{ textAlign: "right", color: C.gold }}>E. Haaland (27 ⚽)</span>
          <span style={{ color: C.muted }}>2025/26 Top Assist</span>
          <span style={{ textAlign: "right", color: C.accent }}>B. Fernandes (21 🅰)</span>
          <span style={{ color: C.muted }}>2024/25 Golden Boot</span>
          <span style={{ textAlign: "right", color: C.muted }}>E. Haaland (27 ⚽)</span>
          <span style={{ color: C.muted }}>2023/24 Golden Boot</span>
          <span style={{ textAlign: "right", color: C.muted }}>E. Haaland (27 ⚽)</span>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={S.app}>
      ...
      {installReady && !installDismissed && (
        <div style={{ background: "#0c1826", borderBottom: "1px solid #1a2d42", padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 22 }}>⚽</span>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: C.text }}>Install EPL Predictor</div>
            <div style={{ fontSize: 10, color: C.muted }}>Add to home screen for the best experience</div>
          </div>
          <button onClick={handleInstall} style={{ background: C.sky, color: "#070e1a", border: "none", borderRadius: 7, padding: "7px 14px", fontWeight: 800, fontSize: 12, cursor: "pointer", flexShrink: 0 }}>Install</button>
      ...
      {showIOSPrompt && !installDismissed && (
        <div style={{ background: "#0a1826", borderBottom: "1px solid #1a2d42", padding: "10px 14px" }}>
          ...
          <div style={{ fontSize: 11, color: C.muted, lineHeight: 1.6 }}>
            Tap <strong style={{ color: C.sky }}>Share</strong> (□↑) at the bottom of Safari, then tap <strong style={{ color: C.sky }}>"Add to Home Screen"</strong> to install as an app.
          </div>
        </div>
      )}

      <div style={S.hdr}>
        <div style={{ fontSize: "clamp(18px,4.5vw,26px)", fontWeight: 900, background: "linear-gradient(90deg,#6cabdd,#ffffff,#6cabdd)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: "-0.5px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 6, fontSize: 10 }}>
          <span style={{ color: C.sky }}>🏆 Arsenal (Champions)</span>
