...
const BUILD_VERSION = "epl2027-v2.0";
...
function SweepstakeView({ liveResults=[] }){
  ...
  const getOrCreateLookup=async()=>{
    ...
    if(stored){
      try{ const r=await fetch(`${JBASE}/b/${stored}/latest`,{headers:jH}); if(r.ok) return{id:stored,data:(await r.json()).record||{}}; }catch{}
    }
  ...
  const updateLookup=async(code,bid)=>{
    try{ const{id,data}=await getOrCreateLookup(); await fetch(`${JBASE}/b/${id}`,{method:"PUT",headers:jH,body:JSON.stringify({...data,[code]:bid})}); }catch{}
  };
  ...
  const refreshRoom=async()=>{
    if(!binId) return;
    try{ const resp=await fetch(`${JBASE}/b/${binId}/latest`,{headers:jH}); setRoom((await resp.json()).record); }catch{}
  };
          ...
          {myRooms.map(r=>(
            <div key={r.code} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderTop:`1px solid ${C.border}`}}>
              ...
              <button onClick={async()=>{
                ...
                try{ const resp=await fetch(`${JBASE}/b/${r.binId}/latest`,{headers:jH}); setRoom((await resp.json()).record); setScreen("room"); }catch{}
                setLoading(false);
...
function PredictView(){
  ...
  const predict=async()=>{
    ...
    }catch(){ setAiText("AI preview unavailable."); }
    setLoading(false);
  };
...
function TopScorersView(){
  ...
  const fetchScorers=async()=>{
    ...
    try{
      ...
      if(resp.ok){
        const data=await resp.json();
        const gL=data?.leaders?.find(l=>l.abbreviation==="G"||l.name?.toLowerCase().includes("goal"));
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    try{ localStorage.setItem(KEY,BUILD_VERSION); }catch{}
    if(!prev) return;
    (async()=>{ try{ if(window.caches?.keys){ const ks=await caches.keys(); await Promise.all(ks.map(k=>caches.delete(k))); } }catch{} window.location.reload(); })();
  },[]);
    ...
    <div style={ST.container}>
      ...
      {tab==="sweep"&&(
        <div style={{background:"linear-gradient(90deg,#050d1a,#0a1530,#050d1a)",borderBottom:`1px solid ${C.border}`,padding:"10px 16px",textAlign:"center"}}>
          <div style={{fontSize:13,fontWeight:800,color:C.sky}}>Be Competitive - Support Your Club - Outsmart Your Friends</div>
        </div>
      )}
